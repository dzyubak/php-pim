...
<title>Sorting items by Priority</title>
...
<h1>Sorting items by Priority</h1>

<form name="sort" action="<?=BASE_URL?>sort_items_by_priority_paginated" method="post">
<p class="center"><input type="radio" name="sort" value="ASC" <?= $order == "ASC" ? 'checked="checked"' : '' ?> /> Ascending
<input type="radio" name="sort" value="DESC" <?= $order == "DESC" ? 'checked="checked"' : '' ?> /> Descending
...
<center>
<?php
echo Paginator::paginatedLinks('sort_items_by_priority_paginated', $total, $offset, $perPage)
