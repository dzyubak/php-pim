...
<title>Filtering items by category</title>
...
<h1>Filtering items by category</h1>

<form name="filter" action="<?=BASE_URL?>filter_items_by_category" method="post">
<p class="center">Select category that should be shown: <select name="category_id">
<?php
foreach( $categoriesArr as $id => $name ) {
        echo '<option value="'.$id.'"';
        if( $id == $categoryId ) { echo ' selected="selected"'; }
        echo '>'.$name.'</option>';
}
?>
</select> <input type="submit" value=" filter " /></p>
</form>
...
<tr>
<?php $i = 0; $lastRowNumber = $offset - 1; ?>
...
<center>
<?php
echo Paginator::paginatedLinks('filter_items_by_category', $total, $offset, $perPage)
        ."<br/>"."(Displaying ".$offset." - ".$lastRowNumber." of ".$total.")";
?>
</center>
</div>
